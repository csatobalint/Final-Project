...
A simplified dynamic model was made which is shown in Figure \ref{robotModel}. Its main purpose to provide a tool that can estimate given states of the robot. This model is not aimed to describe the details of the fall over of the robot. In order to handle it, a timed advanced solution should be provided by integrating the equations over time where due the change of the orientation around the global x and y axis the problem would became nonlinear which solution would be complicated and time-demanding. TODO The mechanical model should be much more detailed to capture properly the changing inertias of robot. The location of contact points would be not trivial to determine too. From practical point of view, it is enough to monitor the sign of the normal force ($N_i$) between the wheels and the ground. If its sign changes to negative from positive then the robot is possible at edge of the fall over. This kind of concept is reasonable since the more significant part of the configuration will be the package and that of the effect on the common center of mass will dominate.??? TODO!
 \begin{figure}[htb!]
	\minipage{0.5\textwidth}
	\centering
...
\begin{itemize}
	...
	\item The package is considered as a cylinder shape object having $\theta_z=\frac{m_p r_p^2}{2}$. The contact between the case and the package is bonded.
\end{itemize}

The equations of Newton's 2nd Law at common center of mass for the case is as follows:
\begin{equation}
	m_c \mathbf{a} = \mathbf{K_A} + \mathbf{K_B} + \mathbf{K_C} + \mathbf{G}
\end{equation}
where \begin{itemize}
	\item $m_c~[\text{kg}]$ is the common mass of the case and the package,
	\item $\mathbf{a}~[\frac{\text{m}}{\text{s}^2}]$ is the acceleration,
	\item $\mathbf{K_i}~[\text{N}]$ is the force transfered at wheel connections, i=(A,B,C),
	\item $\mathbf{G}~[\text{N}]$ is the weight force, $G=g m_R$.
\end{itemize}
\begin{equation}
\boldsymbol{\theta_c} \boldsymbol{\varepsilon} +\boldsymbol{\omega} \times (\boldsymbol{\omega} \times \boldsymbol{\theta_c \boldsymbol{\omega}}) = (\mathbf{r_A}-\mathbf{r_{CoM}}) \times \mathbf{K_A} + (\mathbf{r_B}-\mathbf{r_{CoM}}) \times \mathbf{K_B} + (\mathbf{r_C}-\mathbf{r_{CoM}}) \times  \mathbf{K_C}
\end{equation}
where \begin{itemize}
	\item $\boldsymbol{\theta_c}~[\text{kg m}^2]$ is the common second moment of inertia matrix at the center of mass (CoM), $\boldsymbol{\theta_c} = \boldsymbol{\theta_{Case}} +\boldsymbol{\theta_{Package}} + \boldsymbol{\theta_{MotorA}} + \boldsymbol{\theta_{MotorB}} + \boldsymbol{\theta_{MotorC}} $ (each inertia matrix of a given body is calculated at its own center of mass and then transformed to the common),
...
\begin{figure}[htb!]
	...
	\endminipage\hfill
\end{figure}
Applying the Newton's 3rd Law and taking into consideration the given coordinate system tranformations, the following relationship can be written  regarding the transfered forces:
\begin{equation}
\mathbf{k_i} = -\left[
\begin{array}{ccc}
...
\end{equation}
where $\mathbf{k_i}$ is the force in the joint in the local wheel frames and $i=[A,B,C]$. $\boldsymbol{\sigma}$ is the rotational transformation matrix between the global and local frame. In other words, the components of $\mathbf{k_i}$ will not be new unknowns having the relationship above. Beside the forces in the connection, the gravitational body force and the contact forces ($N,S$) are considered in case of the wheels. As a first approximation, simple one-point contact and no slip condition is assumed which provides 6 kinematic constrain equations:
\begin{flalign}
{a_y}^i&=-r{\varepsilon_y}^i\\
{a_z}^i&=0
\end{flalign}
where $i=[A,B,C]$.\\
In the axial direction not even friction force is considered. In reality in case of an omni wheel, there are rollers in a circumferential alignment which can also spin. In this model, they are modeled as simple frictionless sliders. The possibility of the slippage of wheels are neglected as it would need precise measurements to have a dynamic friction coefficient. On the other hand, the so called rolling resistance can be defined as an adverse torque from point of the motor load torques ($M^i$) which could be taken into consideration in the DC motor equations as loads.\\
...
\end{equation}
where \begin{itemize}
	\item $\boldsymbol{\theta_i}~[\text{kg m}^2]$ is the second moment of inertia matrix of the wheel at the center of mass (CoM), its axial (x) component come froms the DC parameter identification, i.e. the total inertia of the rotating part is considered.
	The other components are calculated as if it was a cylinder with the known geometry and mass. This simplification is only possible because of the fact that the fall over is not wanted to be treated by the model.
	...
	\item $\mathbf{S_i}$ is the static force at the contact of the wheel and ground, while $\mathbf{r_{S_i}}$ is the vector pointing to the contact point from the wheel center of mass $(\mathbf{r_{i}})$.
\end{itemize}
There 24 additional unknowns which are the followings:
\begin{center}
	$\mathbf{a_x}^i,\mathbf{a_y}^i,\mathbf{a_z}^i,\mathbf{\varepsilon_x}^i,\mathbf{\varepsilon_y}^i,\mathbf{\varepsilon_z}^i,S^i,N^i$.
\end{center}
...
The wheel-case connection can assumed to be a rigid body connection in which case the following equations stand and make the closure:
\begin{equation}
...
\end{equation}
The motor loads can be calculated by means of the Eq. \ref{motor_load_equation}, thus the coupling can be done leading to an electrodynamic model. The inputs are velocity states and the voltages at the pins of the motor, while the output is the acceleration state of the robot. The voltages can be calculated by means of the velocity transformation matrix derived in the previous chapter (see Eq. \ref{velocity_tranformation}) which was about the kinematics. This can be generalized since the wheel speed is proportional with the applied voltage. 

The Figure \ref{accelerationState} represent a solution of a simple forward acceleration case. It can be seen that the contact forces of wheel A,B decreases while that of wheel C increases. This phenomenon can be explained by the inertial forces which tries to rotate the case and at the same time the increased contact forces withstand in order to prevent the rotation. In case of too high acceleration compared to the height and the inertias of robot, this phenomenon could lead to fall over. 

The Figure \ref{dangerousOri} depicts the parametric solution of the contact forces with respect to the $\gamma$ angle of the acceleration vector. The dependence can be described with trigonometric functions and it can be concluded that the most dangerous cases, i.e. minimum values of contact forces are present when the robot accelerates such a way that the direction of the acceleration vector is coincident with a single wheel direction. If the model equations calculates negative contact forces that means those are need to prevent the fall over. Negative forces has no real physical sense, they are obtained because of the kinematic constrains but they can be regarded as an indicator for a possible risk for fall over. A possible safety feature can be for such a robot to change its own rotation along a given curve that it takes into consideration the centrifugal acceleration due to the path and tries to set such an orientation where the reduction of contact forces are equally distributed between two wheels instead of one as in the worst case. The angular acceleration does not affect the contact forces based on this model.
\begin{figure}[htb!]
	\minipage{0.45\textwidth}
	\centering
