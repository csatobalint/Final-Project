...
f_2(t,\omega,i)\\
\end{array}
\right]=\mathbf{f}(t,\omega,i)
.
\end{eqnarray}
Let us denote $\mathbf{x}=[\omega,i]$. Applying the discretization by means of Implicit-Euler technique the following equations are obtained:
\begin{eqnarray}
	\left[
	\begin{array}{c}
	F_1(t,\mathbf{x}_{k},\mathbf{x}_{k+1})\\[2ex]
	F_2(t,\mathbf{x}_{k},\mathbf{x}_{k+1})\\
	\end{array}
	\right] = \mathbf{x}_{k+1} - \mathbf{x}_{k} - h \mathbf{f}(t_{k+1},\mathbf{x}_{k+1})
	=\mathbf{F}(t_{k+1},\mathbf{x}_{k},\mathbf{x}_{k+1})
	\label{euler_eq}
\end{eqnarray}
where
\begin{itemize}
	\item $\mathbf{x}_{k}$ is the current time step variable vector,
	\item $\mathbf{x}_{k+1}$ is the next time step variable vector,
	\item $h$ is the time step,
	\item $t_{k+1}$ is time at the $(k+1)^{th}$ timestep
\end{itemize}
Using the Newton's method an inner iteration equation of a given step is as follows:
\begin{equation}
	\mathbf{x_{k+1}}^* = \mathbf{x_{k+1}}-\mathbf{J}^{-1}_{\omega,i}(\mathbf{x}_{k+1})\mathbf{F}(t_{{k+1}},\mathbf{x}_{k},\mathbf{x}_{k+1})  
		\label{newton_eq}
\end{equation}
where
\begin{equation}
\mathbf{J}_{\omega,i}=\grad \mathbf{F}(t_{{k+1}},\mathbf{x}_{k},\mathbf{x}_{k+1}).
\end{equation}
The solution can be calculated by means of \ref{euler_eq} by proceeding in time and at the given time steps the solution of the algebraic equation is obtained by Newton's iteration using the equation \ref{newton_eq}. The $h$ time step was chosen to be 0.001s and the 3 inner iteration were used in equation \ref{newton_eq} which provided sufficiently accurate solution.
Considering the governing equations, it can be said that let us omit the inductance of the armature, thus the "inertia" of the electric circuit. With this assumption combining the two constitutive equations, the following first-order linear differential equation for $\omega[t]$ rotational speed can be derived:
\begin{equation}
